...
public class JlExcel extends AbstractExcelView {
    ...
    public JlExcel(String excelName) {
    ...
    private void createSheet(HSSFWorkbook workbook, String sheetName, String[] title, List dataList) {
        ...
        header0.createCell(0).setCellValue("警力报告表(" + nowTime + ")");
        ...
        // 填充数据
        int rowNum = 2;
        for (Iterator<DeptJL> iter = dataList.iterator(); iter.hasNext(); ) {

            DeptJL element = (DeptJL) iter.next();
            ...
            for (int j = 0; j < title.length; j++) {
                ...
                if (j == 0) {
                    ...
                    row.createCell(j).setCellValue(element.getTimeNodes());

                } else if (j == 1) {

                    row.createCell(j).setCellValue(element.getMjis());

                } else if (j == 2) {

                    row.createCell(j).setCellValue(element.getSydwis());

                } else if (j == 3) {

                    row.createCell(j).setCellValue(element.getZgis());

                } else if (j == 4) {

                    row.createCell(j).setCellValue(element.getWdwis());
