 ...
 */
public class YcExcel extends AbstractExcelView {
    private String excelName;

    public YcExcel(String excelName) {
...
//        date = calendar.getTime();
        ...
        header0.createCell(0).setCellValue("异常数据汇总表("+nowTime+")");
        ...
        for (Iterator iter = dataList.iterator(); iter.hasNext(); ) {
            ...
            for (int j = 0; j < title.length; j++) {
                ...
                if (j == 0) {
                    ...
                    row.createCell(j).setCellValue((String) reqMap.get("name"));

                } else if (j == 1) {

                    row.createCell(j).setCellValue((String) reqMap.get("info"));
