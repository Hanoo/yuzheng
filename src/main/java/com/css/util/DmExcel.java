...
public class DmExcel extends AbstractExcelView {
    private String excelName;

    public DmExcel(String excelName) {
    ...
    private void createSheet(HSSFWorkbook workbook, String sheetName, String[] title, List dataList) {
        ...
        header0.createCell(0).setCellValue("出工情况统计表");
        ...
        for (Iterator iter = dataList.iterator(); iter.hasNext(); ) {
            ...
            Map dmInfo = (Map)iter.next();
            HSSFRow row = sheet.createRow(rowNum++);

            for (int j = 0; j < title.length; j++) {
                // 此处需要调试 匹配对应字段
                if (j == 0) {
                    row.createCell(j).setCellValue(dmInfo.get("name").toString());
                } else if (j == 1) {
                    row.createCell(j).setCellValue(dmInfo.get("pcount").toString());
                } else if (j == 2) {
                    row.createCell(j).setCellValue(dmInfo.get("realCount").toString());
                } else {
